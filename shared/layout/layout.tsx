...
export function LayoutInner({ className, children, ...props }: ComponentProps<'div'>) {
    return <div className={cn("px-4 flex flex-col gap-10", className)} {...props}>{children}</div>
}

export function LayoutHeader({ className, children, ...props }: ComponentProps<'header'>) {
    return <header className={cn("flex flex-col gap-2 items-center justify-center min-h-40", className)} {...props}>{children}</header>
}
